...
export async function GET(request:Request) {
  const {searchParams}=new URL(request.url); const date=clean(searchParams.get('date'),20);
  if(date && !/^\d{4}-\d{2}-\d{2}$/.test(date)) return NextResponse.json({error:'Invalid date.'},{status:400});
  try {
    const db=getSupabaseAdmin(); let query=db.from('bookings').select('preferred_time').in('status',ACTIVE_STATUSES);
...
export async function POST(request:Request) {
  try {
    ...
    if(!service) {
      ...
      if(localService) {
        const {data:createdService,error:serviceError}=await supabase.from('services').upsert({slug:localService.slug,name:localService.name,description:localService.description,active:true},{onConflict:'slug'}).select('id,name,slug').single();
